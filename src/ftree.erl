...
lpush(_Measure, Elem, #ft_empty{}) -> 
	#ft_single{ data = Elem };
lpush(Measure, Elem, #ft_single{data = Existing}) -> 
...
rpush(_Measure, Elem, #ft_empty{}) -> 
	#ft_single{data = Elem};
rpush(Measure, NewElem, #ft_single{data=Elem}) ->
...
%% @doc Traverses the entire structure from left to right, calling Fun with the element and the accumulated result. 
...
foldl(_Fun, Acc, #ft_empty{}) -> 
	Acc;
foldl(Fun, Acc, #ft_single{ data = Elem}) -> 
	Fun(Elem, Acc);
foldl(Fun, Acc, #ft_deep{ left = Left, middle = Middle, right = Right }) -> 
	Acc1 = lists:foldl(fun (X, Result) -> foldl(Fun, Result, X) end, Acc, Left),
	Acc2 = foldl(Fun, Acc1, Middle),
	lists:foldl(fun (X, Result) -> foldl(Fun, Result, X) end, Acc2, Right);
foldl(Fun, Acc, X) -> 
	Fun(X, Acc).
...
first(#ft_single{data = First}) -> First;
first(#ft_deep{left = [First | _]}) -> First.

%% @doc Returns the last element.
last(#ft_single{data = Last}) -> Last;
...
append(Measure, #ft_single{data = Elem}, Elems, Tree) -> 
	lpush(Measure, Elem, lpushlist(Measure, Elems, Tree));
append(Measure, Tree, Elems, #ft_single{data = Elem}) -> 
